...
- `src/` holds the extension logic.
  - Core logic and utility functions are modularized using ES Modules (`import`/`export`).
  - `src/boot.js` is the entry point that initializes the virtualizer and UI.
  - `src/constants.js` defines shared config and state.
  - `src/utils/`, `src/core/`, `src/ui/` contain extracted sub-modules.
  - `src/background.js` is the service worker.
  - `src/popup.html`, `src/popup.css`, `src/popup.js` implement the popup UI.
- `build/` contains the final bundled output (e.g. `content.js`, `background.js`) generated by `esbuild`.
- `manifest.json` is the Chromium manifest; `manifest_firefox.json` targets Firefox (both point to the `build/` directory for scripts).
...
- Run bundler for dev: `npm run dev` (builds and watches `src/` files) or `npm run build` (one-off build).
- Build Firefox package locally: `npm run build:firefox` (automatically runs `npm run build` first).
...
- Use standard ES Modules (`import`/`export`) for all new code. Avoid adding new properties to the global namespace.
...
## Agent Capabilities
- **Code Audits**: Perform code audits regularly to maintain code quality, identify issues, and ensure adherence to best practices.
- **Version Control & Check-ins**: Execute git check-ins, staging tracked changes and creating commits, whenever the user explicitly requests and says the code is "ready" to check in.
- **Task & Artifact Management**: Manage structured tasks via `task.md`, propose technical designs in `implementation_plan.md`, and document testing steps in `walkthrough.md`.
- **Workflow Automation**: Support build processes, CI/CD GitHub action pipelines, Firefox extension signing, and version bumping.

## Agent Maintenance Rule
- Keep `AGENTS.md` up to date whenever agentic capabilities, workflows, scripts, release automation, CI behavior, or required secrets/labels change. This ensures `AGENTS.md` remains continuously updated with any future agentic work.
